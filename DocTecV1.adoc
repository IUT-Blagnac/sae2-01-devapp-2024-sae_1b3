= Document Technique Utilisateur DailyBank - Version : 1.0 - Date : 31 Juin 2024 Groupe 1B3 
:icons: font
:models: models
:experimental:
:incremental:
...
- OperationEditorPaneViewController : Ce contrôleur est responsable de l'affichage et de l'ajout des opérations de virement dans l'interface utilisateur ( la méthode *displayDialog* && la méthode *doAjouter* ) 
...
- EmployeController : Ce contrôleur gère l'action adu chef d'agence pour supprimer un employé en appelant la méthode `supprimerEmploye()`.
